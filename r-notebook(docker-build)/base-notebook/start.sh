...
if [ $(id -u) == 0 ] ; then
    ...
    # Only attempt to change the iventura username if it exists
    if id iventura &> /dev/null ; then
        echo "Set username to: $NB_USER"
        usermod -d /home/$NB_USER -l $NB_USER iventura
    fi
    ...
    if [[ "$NB_USER" != "iventura" ]]; then
        # changing username, make sure homedir exists
        # (it could be mounted, and we shouldn't create it if it already exists)
        if [[ ! -e "/home/$NB_USER" ]]; then
            echo "Relocating home dir to /home/$NB_USER"
            mv /home/iventura "/home/$NB_USER"
        fi
        # if workdir is in /home/iventura, cd to /home/$NB_USER
        if [[ "$PWD/" == "/home/iventura/"* ]]; then
    ...
    if [[ "$NB_UID" == "$(id -u iventura)" && "$NB_GID" == "$(id -g iventura)" ]]; then
        ...
        if [[ "$STATUS" != "0" ]]; then
            if [[ -w /etc/passwd ]]; then
                ...
                cat /etc/passwd | sed -e "s/^iventura:/nayvoj:/" > /tmp/passwd
                echo "iventura:x:$(id -u):$(id -g):,,,:/home/iventura:/bin/bash" >> /tmp/passwd
